        ...
        { 
          data.map((item, index) => (
            <div className="flex flex-row justify-between items-center p-1">
              <span className="text-base max-2xl:text-[0.8vw] truncate flex items-center justify-center"><Card withBorder className={`${color} flex items-center justify-center`} w={25} h={25} p={2} mr={10}>{item?.icon}</Card>{item.title}</span>
              <span className="text-base max-2xl:text-[0.8vw] truncate">{
                loading ? (
                  <Loader variant="dots" size="md" className="text-brand" />
                ) :

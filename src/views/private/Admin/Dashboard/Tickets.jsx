  ...
  const columns = [
    {
      ...
      selector: 'id',
      cell: (row) =>(<span className="max-2xl:text-[0.8vw]"> {row.id.toString().padStart(6, "0") } </span>),
      sortable: true,
    },
    {
    ...
    },
    {
      ...
      selector: 'requester',
      cell: (row) =>(<span className="max-2xl:text-[0.8vw]"> {row.gpUser.vorname + " " + row.gpUser.nachname  } </span>),
      sortable: true,
    },
    {
      name: <strong className="max-2xl:text-xs">{t('Problem Type')}</strong>,
      selector: 'problemType',
      cell: (row) =>(<span className="max-2xl:text-[0.8vw]">{ row.ticketCategory.name}</span>),
      sortable: true,
    },
    {
      name: <strong className="max-2xl:text-xs">{t('Title')}</strong>,
      selector: 'title',
      cell: (row) =>(<span className="max-2xl:text-[0.8vw]">{ row.title}</span>),
      sortable: true,
    },
    {
      name: <strong className="max-2xl:text-xs">{t('Priority')}</strong>,
      selector: 'priority',
      cell: (row) =>(<span className="max-2xl:text-[0.8vw]"> {row.ticketPriority.name}</span>),
      sortable: true,
    },
    {
      name: <strong className="max-2xl:text-xs">{t('Created')}</strong>,
      selector: 'created_at',
      cell: (row) =>(<span className="max-2xl:text-[0.8vw]"> {new Date(row.created_at).toLocaleDateString().replaceAll("/", ".")}</span>),
      sortable: true,
    },
    {
      name: <strong className="max-2xl:text-xs">{t('Updated')}</strong>,
      selector: 'updated_at',
      cell: (row) =>(<span className="max-2xl:text-[0.8vw]"> {new Date(row.updated_at).toLocaleDateString().replaceAll("/", ".")}</span>),
      sortable: true,
    },
    {
      name: '',

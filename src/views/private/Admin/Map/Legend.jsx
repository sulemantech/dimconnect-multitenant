                ...
                {Object.entries(legendContent.value)?.map(([key, item]) => {
                  return (
                    ...
                    >
                      ...
                      <p
                        className={`text-xs ${
                          addressPointsStatusVisibility.value[item.code] ? "text-gray-900" : "text-gray-400"
                        }`}
                      >
                        {key}
                      </p>
                      ...
                      {Object.entries(item)?.map(([key, item]) => {
                        return (
                          ...
                          >
                            ...
                            <p
                              className={`text-xs ${
                                DistrictPhaseLayersVisibility.value[item.key] ? "text-gray-900" : "text-gray-400"
                              }`}
                            >
                              {key}
                            </p>
...
const NetzplanningAccordionItem = () => {
    const netzplanningItems = [
      {
        key: "DemandPoints",
        color: "red",
        label: "Demand Points",
      ...
      {
        key: "PrimaryDistribtion",
        color: "blue",
        label: "PrimDistribtion Cable",
        shape: "line",
      },
    ];
          ...
          <div>
            {netzplanningItems.map((item) => (
              ...
              >
                ...
                {item.shape === "line" && (
                  <div
                    className="w-4 border-l border-blue-500 mr-2"
                    style={{ height: "1px" }}
                  ></div>
                )}
                {item.shape === "icon" && (
                  <div className="w-4 h-4 mr-2">
                    {/* Insert icon component or SVG here */}
                  </div>
                )}
                <div className="flex-1" />

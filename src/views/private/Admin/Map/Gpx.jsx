        ...
        <>
          {Object.keys(data).map((key) => (
            <>
              {data[key]
                .filter(
                  (item) => parseFloat(item[3]) > 0 && parseFloat(item[4]) > 0
                )
                ?.map((item, index) => (
                  <Marker
                    key={"efe0" + index}
                      ...
                      onClick={(e) => {
                        ...
                        getGPX(item[0])
                          .then(({ data }) => {
                            ...
                            convertedData.video = item[2]
                              .replace("resources/media/uploads/gpx/", "")
                              .replace(".gpx", ".mp4");
                            openModal({
                              children: (
                                <>
                                  <video
                                    ...
                                    src={`${appConfig.mediaServerURL}/${convertedData.video}`}
                                    type="video/mp4"
                                  />
                                </>
                              ),
...
const Pin = memo((props) => {
  ...
  return (
    <div
      ...
      className="relative flex flex-col items-center cursor-pointer hover:scale-125 transition-all justify-center"
    >
          ...
          {data.features
            ...
            ?.map((item, index) => {
              if (item.geometry.type === "Point")
                          ...
                          children: (
                            <>
                              <video
                                ...
                                className="h-full w-full"
                                src={`${appConfig.mediaServerURL}/${data.video}`}
                                type="video/mp4"
                              />
                            </>
                          ),
                ...
                return item.geometry.coordinates?.map((item, index) => {
                  return (
                    <Marker
                      key={"efe0fef" + index}
                      latitude={item[1]}

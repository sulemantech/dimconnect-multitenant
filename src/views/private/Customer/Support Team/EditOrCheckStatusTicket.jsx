        ...
        <div className="w-full h-full  bg-white">
          <div className="p-12">
            <Title order={2} className="mb-8" color="brand">
              {t("Edit or Check Status of your Tickets Here")}
            </Title>
            <p className="text-xs">
              {t("You can")} <b>{t("Edit")}</b> {t("or")} <b>{t("Delete")}</b>{" "}
              {t("your Support Ticket easy, before it gets status")}{" "}
              <b>{t("SOLVED")}</b>.{" "}
              {t(
            ...
            <section>
              <div className="flex flex-row my-6 items-center justify-between mt-8 text-xs">
                <p>{t("History of your Support Tickets")}</p>
                <Input icon={<IconSearch size={20} />} variant="filled" />
              </div>
            </section>
            {data ? (
              <TicketDetails data={data} getTicketData={getTicketData} />
            ) : (
              <Loader />
            )}
          </div>
          ...
          <Accordion.Panel className="w-full">
            <p className=" flex flex-col space-y-2 ml-[3.8rem] my-2 text-[0.7rem] ">
              <div className="flex flex-1 space-x-6">
                <p className=" font-medium">{t("Problem Type")}</p>{" "}
    ...
    >
      <Accordion.Control className="bg-[#F5F7F9] h-[1.3rem]">
        ...
        <div className="flex  ml-[37px] space-x-[38px] flex-1 ">
          <div className="flex  flex-1">
            <p className=" flex space-x-2 text-[0.7rem] flex-1">
              <p className=" font-medium">{t("Your Reply")}</p>{" "}
              <span className=" font-medium">
                {comment.ticket_id}R{comment.id}
              </span>
            </p>{" "}

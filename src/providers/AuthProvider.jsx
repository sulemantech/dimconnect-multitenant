...
const createAuthState = () => {
  ...
  Cookies.get(appConfig.cookieKey) ? auth.value = true : auth.value = false;

  if (!auth.value) {
    return { auth, setAuth };
  }

  api.defaults.headers.common['authorization'] = `Bearer ${Cookies.get(appConfig.cookieKey)}`; // Replace localStorage with Cookies
  const jwt = jwtDecode(Cookies.get(appConfig.cookieKey)); // Replace localStorage with Cookies
  ...
  getRegionList().then(({ data }) => {
    ...
    auth.value = false;
    Cookies.remove(appConfig.cookieKey); // Replace localStorage with Cookies
  });
  ...
  const timer = () => setTimeout(() => {
    if (auth.value && Cookies.get(appConfig.cookieKey)) { // Replace localStorage with Cookies
      refreshAuth(Cookies.get(appConfig.localStorageRefreshKey)).then(res => { // Replace localStorage with Cookies
        Cookies.set(appConfig.cookieKey, res.data.token, { secure: true, httpOnly: true }); // Replace localStorage with Cookies
        api.defaults.headers.common['authorization'] = `Bearer ${res.data.token}`;
        showNotification({
          title: 'Session refreshed',
          message: 'Your session has been refreshed',
        ...
        showNotification({
          ...
          timeout: 5000,
          onClose: () => {
            Cookies.remove(appConfig.localStorageRefreshKey); // Replace localStorage with Cookies
            Cookies.remove(appConfig.cookieKey); // Replace localStorage with Cookies
            auth.value = false;
          }
        });

...
const RouteComponent = ({ path, label, icon, subroutes, isSubRoute = false }) => {
    ...
    return (
        <Link href={path}>
            <Tooltip disabled={collapsed.value || isSubRoute} label={<div className="flex  flex-1 items-center text-sm ml-6 mr-4 truncate font-[400]">{icon} <p className="text-sm pl-[40px] text-center justify-center flex  flex-1 truncate font-[400]"> {t(label)}</p></div>} openDelay={0} position="right" classNames={{
                tooltip: 'flex items-center justify-start bg-opacity-80 backdrop-blur-sm  bg-[#0E76BB]  border-b-2 border-[#0000005e] text-white rounded-r-3xl rounded-l-none -ml-[100px] pl-[30px] h-[73.5px] max-2xl:h-[60px] min-w-[200px]',
